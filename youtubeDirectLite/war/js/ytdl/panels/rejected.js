    ...
    init: function () {
      $('body').on({
        click: function() {
          ...
          utils.addVideoToPlaylist(globals.hashParams.playlist, button.attr('data-video-id'));
      ...
      utils.getPlaylists(function() {
        utils.getStateOfSubmissions(function(state) {
          if (state) {
            ...
            $.each(state.rejectedIds, function() {
              var metadata = state.videoIdToMetadata[this];
              if (metadata) {
                var editUrl = state.videoIdToPlaylistEntryId[this];
                metadata.buttonsHtml = utils.format('<input type="button" class="approve" value="Approve" data-video-id="{videoId}" data-edit-url="{editUrl}">', {
                  videoId: this,
                  editUrl: editUrl
                });
            ...
            $('#rejected-videos').html(lis.join(''));

            $('#moderation-message').text(utils.format('{0} {1} rejected.', lis.length, lis.length == 1 ? 'video is' : 'videos are')).show();
          } else {
